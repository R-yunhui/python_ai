...
重要概念：
1. 流式输出 + 工具调用需要特殊处理！
   - 不能直接在 stream 中获取 tool_calls
   - 需要累积完整响应后才能检测工具调用

2. 工具调用的正确流程（重要！）：
   ① 执行工具，获取原始结果
   ② 将结果添加到对话历史（ToolMessage）
   ③ 让AI基于工具结果生成友好、专业的回复 ⭐
   
   ❌ 错误做法：直接将工具的原始结果返回给用户
   ✅ 正确做法：让AI对工具结果进行整理、润色、个性化回复
"""
    ...
    for i, question in enumerate(questions, 1):
        ...
        if hasattr(ai_message, 'tool_calls') and ai_message.tool_calls:
            ...
            for tool_call in ai_message.tool_calls:
                ...
                # 📝 教学展示：显示工具原始结果
                # ⚠️  注意：在实际项目中，不应该直接显示原始结果
                # 而是让AI整理后再呈现给用户
                print(f"  原始结果: {tool_result[:100]}...")  # 只显示前100字符
